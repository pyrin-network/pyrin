...
use pyrin_daemon::{DaemonEvent, DaemonKind, Daemons};
use pyrin_wallet_core::rpc::DynRpcApi;
use pyrin_wallet_core::storage::{IdT, PrvKeyDataInfo};
use pyrin_wrpc_client::PyrinRpcClient;
...
impl From<&PyrinCli> for Arc<Terminal> {
    fn from(ctx: &PyrinCli) -> Arc<Terminal> {
        ctx.term()
    }
}

impl AsRef<PyrinCli> for PyrinCli {
    fn as_ref(&self) -> &Self {
        self
    }
}

impl workflow_log::Sink for PyrinCli {
...
impl PyrinCli {
    pub fn init() {
        cfg_if! {
            if #[cfg(not(target_arch = "wasm32"))] {
                ...
                pyrin_core::log::init_logger(None, "info");
            } else {
                pyrin_core::log::set_log_level(LevelFilter::Info);
            }
        }
    ...
    pub async fn try_new_arc(options: Options) -> Result<Arc<Self>> {
        ...
        let pyrin_cli = Arc::new(PyrinCli {
        ...
        let term = Arc::new(Terminal::try_new_with_options(pyrin_cli.clone(), options.terminal)?);
        term.init().await?;

        cfg_if! {
            if #[cfg(target_arch = "wasm32")] {
                pyrin_cli.init_panic_hook();
            }
        }
    ...
    pub fn try_rpc_client(&self) -> Option<Arc<PyrinRpcClient>> {
                ...
                select! {
                    ...
                    msg = multiplexer.receiver.recv().fuse() => {
                        ...
                        if let Ok(msg) = msg {
                            match *msg {
                                Events::WalletPing => {
                                    // log_info!("Pyrin NG - received wallet ping");
                                },
                                Events::Metrics { network_id : _, metrics : _ } => {
                                    // log_info!("Pyrin NG - received metrics event {metrics:?}")
                                }
                                ...
                                Events::UtxoIndexNotEnabled { .. } => {
                                    tprintln!(this, "Error: Pyrin node UTXO index is not enabled...")
                                },
                                ...
                                Events::ServerStatus {
                                    ...
                                    tprintln!(this, "Connected to Pyrin node version {server_version} at {}", url.unwrap_or("N/A".to_string()));

                                    let is_open = this.wallet.is_open();

                                    if !is_synced {
                                        if is_open {
                                            terrorln!(this, "Unable to update the wallet state - Pyrin node is currently syncing with the network...");

                                        } else {
                                            terrorln!(this, "Pyrin node is currently syncing with the network, please wait for the sync to complete...");
                                        }
                                    }
    ...
    pub async fn shutdown(&self) -> Result<()> {
        if !self.shutdown.load(Ordering::SeqCst) {
            ...
            let pyrind = self.daemons().try_pyrind();
            ...
            if let Some(pyrind) = pyrind.as_ref() {
                pyrind.mute(false).await?;
                pyrind.stop().await?;
            }
...
impl Cli for PyrinCli {
    ...
    fn prompt(&self) -> Option<String> {
        ...
        if let Some(descriptor) = self.wallet.descriptor() {
            ...
            if title.to_lowercase().as_str() != "pyrin" {
                prompt.push(title);
            }
...
pub async fn pyrin_cli(terminal_options: TerminalOptions, banner: Option<String>) -> Result<()> {
    PyrinCli::init();

    let options = Options::new(terminal_options, None);
    let cli = PyrinCli::try_new_arc(options).await?;

    let banner =
        banner.unwrap_or_else(|| format!("Pyrin Cli Wallet v{} (type 'help' for list of commands)", env!("CARGO_PKG_VERSION")));

use crate::imports::*;
use pyrin_consensus_core::tx::{TransactionInput, TransactionOutpoint};
use pyrin_wallet_core::storage::Binding;
use pyrin_wallet_core::storage::{TransactionData, TransactionKind, TransactionRecord};
...
impl TransactionExtension for TransactionRecord {
    ...
    async fn format_transaction_with_args(
        ...
        let suffix = pyrin_suffix(&self.network_id.network_type);
        ...
        match transaction_data {
            ...
            | TransactionData::Change { utxo_entries, aggregate_input_value, .. } => {
                let aggregate_input_value =
                    transaction_type.style_with_sign(sompi_to_pyrin_string(*aggregate_input_value).as_str(), history);
                ...
                if include_utxos {
                    for utxo_entry in utxo_entries {
                        ...
                        let amount = transaction_type.style_with_sign(sompi_to_pyrin_string(utxo_entry.amount).as_str(), history);
            ...
            | TransactionData::TransferOutgoing { fees, aggregate_input_value, transaction, payment_value, change_value, .. } => {
                if let Some(payment_value) = payment_value {
                    lines.push(format!(
                        ...
                        "",
                        style(sompi_to_pyrin_string(*payment_value)).red(),
                        style(sompi_to_pyrin_string(*aggregate_input_value)).blue(),
                        style(sompi_to_pyrin_string(*fees)).red(),
                        style(sompi_to_pyrin_string(*change_value)).green(),

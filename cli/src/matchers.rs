...
pub fn register_link_matchers(cli: &Arc<PyrinCli>) -> Result<()> {
    if !is_wasm() {
        return Ok(());
    }
    ...
    // addresses (open,copy) https://explorer.pyrin.org/addresses/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        &js_sys::RegExp::new(r"(pyrin|pyrintest):\S+", "i"),
        Arc::new(Box::new(move |modifiers, uri| {
            if modifiers.ctrl || modifiers.meta {
                if uri.starts_with("pyrintest") {
                    cli_.term().writeln("testnet addresses can not be currently looked up with the block explorer");
                } else {
                    let url = format!("https://explorer.pyrin.org/addresses/{uri}");
                    if is_nw() {
                        nw_sys::shell::open_external(&url);
                    } else {
                        link::open(&url);
    ...
    // blocks (open,copy) https://explorer.pyrin.org/blocks/
    let cli_ = cli.clone();
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.pyrin.org/blocks/{uri}"));
            } else {
    ...
    cli.term().register_link_matcher(
        ...
        Arc::new(Box::new(move |modifiers, text| {
            ...
            if modifiers.ctrl || modifiers.meta {
                nw_sys::shell::open_external(&format!("https://explorer.pyrin.org/txs/{uri}"));
            } else {
                write_to_clipboard(&cli_, uri.to_string().as_str());
            }
        })),
...
fn write_to_clipboard(cli: &Arc<PyrinCli>, text: &str) {
    if is_nw() {

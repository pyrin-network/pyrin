...
use pyrin_bip32::secp256k1::XOnlyPublicKey;
use pyrin_wallet_core::{
...
impl Message {
    async fn main(self: Arc<Self>, ctx: Arc<PyrinCli>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
        ...
        match argv.first().unwrap().as_str() {
            ...
            "verify" => {
                ...
                self.verify(ctx, pyrin_address, signature, message).await?;
            }
            v => {
    ...
    async fn display_help(self: Arc<Self>, ctx: Arc<PyrinCli>, _argv: Vec<String>) -> Result<()> {
        ctx.term().help(
            &[
                ("sign <pyrin_address>", "Sign a message with the private key that matches the given address. Prompts for message."),
                (
                    "verify <pyrin_address> <signature>",
                    "Verify the signature against the message and pyrin_address. Prompts for message.",
                ),
            ],
            None,
        )?;
    ...
    async fn sign(self: Arc<Self>, ctx: Arc<PyrinCli>, pyrin_address: &str, message: &str) -> Result<()> {
        let pyrin_address = Address::try_from(pyrin_address)?;
        if pyrin_address.version != Version::PubKey {
        ...
        let privkey = self.get_address_private_key(&ctx, pyrin_address).await?;
    ...
    async fn verify(self: Arc<Self>, ctx: Arc<PyrinCli>, pyrin_address: &str, signature: &str, message: &str) -> Result<()> {
        let pyrin_address = Address::try_from(pyrin_address)?;
        if pyrin_address.version != Version::PubKey {
            return Err(Error::custom("Address not supported for message signing. Only supports PubKey addresses"));
        }

        let pubkey = XOnlyPublicKey::from_slice(&pyrin_address.payload[0..32]).unwrap();
    ...
    async fn get_address_private_key(self: Arc<Self>, ctx: &Arc<PyrinCli>, pyrin_address: Address) -> Result<[u8; 32]> {
        ...
        match account.account_kind().as_ref() {
            BIP32_ACCOUNT_KIND => {
                ...
                let (receive, change) = account.derivation().addresses_indexes(&[&pyrin_address])?;

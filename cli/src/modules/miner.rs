use crate::imports::*;
use pyrin_daemon::{locate_binaries, CpuMinerConfig};
...
impl DefaultSettings for MinerSettings {
    async fn defaults() -> Vec<(Self, Value)> {
        ...
        if let Ok(binaries) = locate_binaries(&root, "pyrin-cpu-miner").await {
...
impl Miner {
    ...
    async fn create_config(&self, ctx: &Arc<PyrinCli>) -> Result<CpuMinerConfig> {
    ...
    async fn main(self: Arc<Self>, ctx: Arc<PyrinCli>, mut argv: Vec<String>, _cmd: &str) -> Result<()> {
        if argv.is_empty() {
    ...
    async fn select(self: Arc<Self>, ctx: Arc<PyrinCli>) -> Result<()> {
        let root = nw_sys::app::folder();

        let binaries = pyrin_daemon::locate_binaries(root.as_str(), "pyrin-cpu-miner").await?;

        if binaries.is_empty() {
            tprintln!(ctx, "No pyrin-cpu-miner binaries found");
        } else {
            let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
            if let Some(selection) = ctx.term().select("Please select pyrin-cpu-miner binary", &binaries).await? {

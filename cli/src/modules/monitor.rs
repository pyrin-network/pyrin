...
impl Monitor {
    async fn main(self: Arc<Self>, ctx: &Arc<PyrinCli>, _argv: Vec<String>, _cmd: &str) -> Result<()> {
        let max_events = 16;
    ...
    async fn redraw(self: &Arc<Self>, ctx: &Arc<PyrinCli>, events: &Arc<Mutex<VecDeque<Box<Events>>>>) -> Result<()> {
        tprint!(ctx, "{}", ClearScreen);
        tprint!(ctx, "{}", Goto(1, 1));
        ...
        if !wallet.is_connected() {
            ...
            tprintln!(ctx, "{}", style("Pyrin node is currently syncing").magenta());
            tprintln!(ctx);
        }

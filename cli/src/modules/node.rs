...
pub struct Node {
    settings: SettingsStore<PyrindSettings>,
    mute: Arc<AtomicBool>,
...
impl Handler for Node {
    fn verb(&self, ctx: &Arc<dyn Context>) -> Option<&'static str> {
        if let Ok(ctx) = ctx.clone().downcast_arc::<PyrinCli>() {
            ctx.daemons().clone().pyrind.as_ref().map(|_| "node")
        } else {
            None
        }
    }
...
impl Node {
    ...
    async fn create_config(&self, ctx: &Arc<PyrinCli>) -> Result<PyrindConfig> {
        let location: String = self
            .settings
            .get(PyrindSettings::Location)
        ...
        let mute = false;
        let config = PyrindConfig::new(location.as_str(), network_id, mute);
        Ok(config)
    }

    async fn main(self: Arc<Self>, ctx: Arc<PyrinCli>, mut argv: Vec<String>, cmd: &str) -> Result<()> {
        if argv.is_empty() {
            return self.display_help(ctx, argv).await;
        }
        let pyrind = ctx.daemons().pyrind();
        match argv.remove(0).as_str() {
            "start" => {
                let mute = self.mute.load(Ordering::SeqCst);
                if mute {
                    tprintln!(ctx, "starting pyrin node... {}", style("(logs are muted, use 'node mute' to toggle)").dim());
                } else {
                    tprintln!(ctx, "starting pyrin node... {}", style("(use 'node mute' to mute logging)").dim());
                }

                let wrpc_client = ctx.wallet().try_wrpc_client().ok_or(Error::custom("Unable to start node with non-wRPC client"))?;

                pyrind.configure(self.create_config(&ctx).await?).await?;
                pyrind.start().await?;
            ...
            "mute" | "logs" => {
                ...
                // pyrind.mute(mute).await?;
                self.settings.set(PyrindSettings::Mute, mute).await?;
            }
            "status" => {
                let status = pyrind.status().await?;
                tprintln!(ctx, "{}", status);
            }
            "select" => {
            ...
            "version" => {
                pyrind.configure(self.create_config(&ctx).await?).await?;
                let version = pyrind.version().await?;
    ...
    async fn select(self: Arc<Self>, ctx: Arc<PyrinCli>, path: Option<String>) -> Result<()> {
        let root = nw_sys::app::folder();

        match path {
            None => {
                let binaries = pyrin_daemon::locate_binaries(root.as_str(), "pyrin").await?;

                if binaries.is_empty() {
                    tprintln!(ctx, "No pyrin binaries found");
                } else {
                    let binaries = binaries.iter().map(|p| p.display().to_string()).collect::<Vec<_>>();
                    if let Some(selection) = ctx.term().select("Please select a pyrind binary", &binaries).await? {
                        tprintln!(ctx, "selecting: {}", selection);
                        self.settings.set(PyrindSettings::Location, selection.as_str()).await?;
                    } else {
            ...
            Some(path) => {
                if fs::exists(&path).await? {
                    ...
                    self.settings.set(PyrindSettings::Location, path.as_str()).await?;
                } else {
                    twarnln!(ctx, "destination binary not found, please specify full path including the binary name");
                    twarnln!(ctx, "example: 'node select /home/user/testnet/pyrind'");
                    tprintln!(ctx, "no selection is made");
                }
            }
        }
    ...
    pub async fn handle_event(&self, ctx: &Arc<PyrinCli>, event: Event) -> Result<()> {
        let term = ctx.term();

...
#[help("Send a Pyrin transaction to a public address")]
pub struct Send;
...
impl Send {
    async fn main(self: Arc<Self>, ctx: &Arc<dyn Context>, argv: Vec<String>, _cmd: &str) -> Result<()> {
        ...
        let amount_sompi = try_parse_required_nonzero_pyrin_as_sompi_u64(argv.get(1))?;
        let priority_fee_sompi = try_parse_optional_pyrin_as_sompi_i64(argv.get(2))?.unwrap_or(0);
        ...
        // tprintln!(ctx, "\nSending {} PYI to {address}, tx ids:", sompi_to_pyrin_string(amount_sompi));

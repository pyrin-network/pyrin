...
use pyrin_wallet_core::storage::{make_filename, Hint};

pub(crate) async fn create(ctx: &Arc<PyrinCli>, name: Option<&str>, import_with_mnemonic: bool) -> Result<()> {
    let term = ctx.term();
    ...
    if let Err(err) = wallet.network_id() {
        tprintln!(ctx);
        tprintln!(ctx, "Before creating a wallet, you need to select a Pyrin network.");
    ...
    if !import_with_mnemonic {
        ...
        tpara!(
            ...
            the Pyrin stored in it. Keep your mnemonic safe. Write it down and \

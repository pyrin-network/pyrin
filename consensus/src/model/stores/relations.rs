...
use pyrin_consensus_core::{blockhash::BlockHashes, BlockHashMap, BlockHasher, BlockLevel};
use pyrin_database::prelude::{BatchDbWriter, CachePolicy, DbWriter};
use pyrin_database::prelude::{CachedDbAccess, DbKey, DirectDbWriter};
use pyrin_database::prelude::{DirectWriter, MemoryWriter};
use pyrin_database::prelude::{ReadLock, StoreError};
use pyrin_database::prelude::{StoreResult, DB};
use pyrin_database::registry::{DatabaseStorePrefixes, SEPARATOR};
...
impl RelationsStoreReader for StagingRelationsStore<'_> {
    ...
    fn counts(&self) -> Result<(usize, usize), StoreError> {
        let count = self
            ...
            .iterator()
            .map(|r| r.unwrap().0)
            .map(|k| <[u8; pyrin_hashes::HASH_SIZE]>::try_from(&k[..]).unwrap())
            .map(Hash::from_bytes)
...
mod tests {
    ...
    use pyrin_utils::mem_size::MemMode;
    ...
    fn test_db_relations_store() {
        let (lt, db) = create_temp_db!(pyrin_database::prelude::ConnBuilder::default().with_files_limit(10));

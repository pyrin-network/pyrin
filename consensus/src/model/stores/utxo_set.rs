...
use pyrin_database::prelude::{CachePolicy, StoreError};
...
pub const UTXO_KEY_SIZE: usize = pyrin_hashes::HASH_SIZE + std::mem::size_of::<TransactionIndexType>();
...
impl AsRef<[u8]> for UtxoKey {
    fn as_ref(&self) -> &[u8] {
        ...
        let rposition = self.0[pyrin_hashes::HASH_SIZE..].iter().rposition(|&v| v != 0).unwrap_or(0);
        &self.0[..=pyrin_hashes::HASH_SIZE + rposition]
    }
}
...
impl TryFrom<&[u8]> for UtxoKey {
    ...
    fn try_from(slice: &[u8]) -> Result<Self, Self::Error> {
        ...
        if slice.len() < pyrin_hashes::HASH_SIZE + 1 {
...
impl From<TransactionOutpoint> for UtxoKey {
    fn from(outpoint: TransactionOutpoint) -> Self {
        let mut bytes = [0; UTXO_KEY_SIZE];
        bytes[..pyrin_hashes::HASH_SIZE].copy_from_slice(&outpoint.transaction_id.as_bytes());
        bytes[pyrin_hashes::HASH_SIZE..].copy_from_slice(&outpoint.index.to_le_bytes());
        Self(bytes)
    }
}
...
impl From<UtxoKey> for TransactionOutpoint {
    fn from(k: UtxoKey) -> Self {
        let transaction_id = Hash::from_slice(&k.0[..pyrin_hashes::HASH_SIZE]);
        let index = TransactionIndexType::from_le_bytes(
            <[u8; std::mem::size_of::<TransactionIndexType>()]>::try_from(&k.0[pyrin_hashes::HASH_SIZE..])

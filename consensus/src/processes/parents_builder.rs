...
use pyrin_consensus_core::{blockhash::ORIGIN, header::Header, BlockHashMap, BlockHasher, BlockLevel};
...
mod tests {
    ...
    impl HeaderStoreReader for HeaderStoreMock {
        fn get_daa_score(&self, hash: pyrin_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_timestamp(&self, hash: pyrin_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_bits(&self, hash: pyrin_hashes::Hash) -> Result<u32, StoreError> {
            unimplemented!()
        }

        fn get_header(&self, hash: pyrin_hashes::Hash) -> Result<Arc<Header>, StoreError> {
            Ok(self.map.read().get(&hash).unwrap().header.clone())
        }

        fn get_compact_header_data(
            &self,
            hash: pyrin_hashes::Hash,
        ) -> Result<crate::model::stores::headers::CompactHeaderData, StoreError> {
            unimplemented!()
        }

        fn get_blue_score(&self, hash: pyrin_hashes::Hash) -> Result<u64, StoreError> {
            unimplemented!()
        }

        fn get_header_with_block_level(&self, hash: pyrin_hashes::Hash) -> Result<HeaderWithBlockLevel, StoreError> {

...
use pyrin_math::int::SignedInteger;
...
use pyrin_core::{debug, info, trace};
use pyrin_database::prelude::{CachePolicy, ConnBuilder, StoreResultEmptyTuple, StoreResultExtensions};
use pyrin_hashes::Hash;
use pyrin_pow::calc_block_level;
use pyrin_utils::{binary_heap::BinaryHeapExtensions, vec::VecExtensions};
use thiserror::Error;
use pyrin_consensus_core::config::params::MAINNET_PARAMS;
use crate::{
    consensus::{
...
impl PruningProofManager {
    ...
    pub fn import_pruning_points(&self, pruning_points: &[Arc<Header>]) {
        for (i, header) in pruning_points.iter().enumerate() {
            ...
            let state = pyrin_pow::State::new(header);
    ...
    pub fn validate_pruning_point_proof(&self, proof: &PruningPointProof) -> PruningImportResult<()> {
        ...
        let (db_lifetime, db) = pyrin_database::create_temp_db!(ConnBuilder::default().with_files_limit(10));
        ...
        for anticone_block in anticone.iter().copied() {
            ...
            for hash in ghostdag_chain {
                if let Entry::Vacant(e) = ghostdag_blocks.entry(hash) {
                    ...
                    // We fill `ghostdag_blocks` only for pyrind-go legacy reasons, but the real set we
                    // send is `daa_window_blocks` which represents the full trusted sub-DAG in the antifuture
                    // of the pruning point which pyrind-rust nodes expect to get when synced with headers proof
                    if let Entry::Vacant(e) = daa_window_blocks.entry(hash) {
                        e.insert(TrustedHeader {
                            header: self.headers_store.get_header(hash).unwrap(),

...
impl TransactionValidator {
    ...
    fn check_coinbase_in_isolation(&self, tx: &pyrin_consensus_core::tx::Transaction) -> TxResult<()> {
        if !tx.is_coinbase() {
            return Ok(());
        }
...
fn check_gas(tx: &Transaction) -> TxResult<()> {
    // This should be revised if subnetworks are activated (along with other validations that weren't copied from pyrind)
    if tx.gas > 0 {

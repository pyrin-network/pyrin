...
/// Address prefix identifying the network type this address belongs to (such as `pyrin`, `pyrintest`, `pyrinsim`, `pyrindev`).
...
///
///  Pyrin `Address` version (`PubKey`, `PubKey ECDSA`, `ScriptHash`)
///
/// @category Address
#[derive(PartialEq, Eq, PartialOrd, Ord, Clone, Copy, Debug, Hash, Serialize, Deserialize, BorshSerialize, BorshDeserialize)]
#[repr(u8)]
...
/// Pyrin `Address` struct that serializes to and from an address format string: `pyrin:qz0s...t8cv`.
/// @category Address

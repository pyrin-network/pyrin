...
use pyrin_consensus_core::hashing::sighash::{calc_ecdsa_signature_hash, calc_schnorr_signature_hash, SigHashReusedValues};
use pyrin_consensus_core::hashing::sighash_type::SigHashType;
use pyrin_consensus_core::tx::{ScriptPublicKey, TransactionInput, UtxoEntry, VerifiableTransaction};
...
impl<'a, T: VerifiableTransaction> TxScriptEngine<'a, T> {
    ...
    fn execute_opcode(&mut self, opcode: Box<dyn OpCodeImplementation<T>>) -> Result<(), TxScriptError> {
        // Different from pyrind: Illegal and disabled opcode are checked on execute instead
        // Note that this includes OP_RESERVED which counts as a push operation.
...
mod bitcoind_tests {
    ...
    use pyrin_consensus_core::constants::MAX_TX_IN_SEQUENCE_NUM;
    use pyrin_consensus_core::tx::{
        PopulatedTransaction, ScriptPublicKey, Transaction, TransactionId, TransactionOutpoint, TransactionOutput,
    };

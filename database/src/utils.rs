...
pub fn get_pyrin_tempdir() -> TempDir {
    let global_tempdir = std::env::temp_dir();
    let pyrin_tempdir = global_tempdir.join("pyrin");
    std::fs::create_dir_all(pyrin_tempdir.as_path()).unwrap();
    let db_tempdir = tempfile::tempdir_in(pyrin_tempdir.as_path()).unwrap();
    db_tempdir
}

/// Creates a DB within a temp directory under `<OS SPECIFIC TEMP DIR>/pyrin-rust`
/// Callers must keep the `TempDbLifetime` guard for as long as they wish the DB to exist.
#[macro_export]
macro_rules! create_temp_db {
    ($conn_builder: expr) => {{
        let db_tempdir = $crate::utils::get_pyrin_tempdir();

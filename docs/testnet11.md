...
*Testnet 11* is the network where we will launch the first public 10BPS Pyrin experiment. The goal of this document is to provide a quick guide for anyone who wants to participate.
...
2.  *pyrin-miner* - the Pyrin CPU miner
...
The recommended hardware requirements are 16GB of RAM, preferably a CPU with at least 8 cores, and an SSD drive with at least 250GB of free space (the 300GB for a safety margin is preferable). It might also be possible to operate with 8GB of RAM, by passing the `--ram-scale=0.6` parameter to `pyrind`.
...
2. Start the ``pyrind`` client with ``utxoindex`` enabled:

```
pyrind --testnet --netsuffix=11 --utxoindex
```
...
```
cargo run --bin pyrind --release -- --testnet --netsuffix=11 --utxoindex
```
...
2. The output will provide you with a private key (that looks like a bunch of gibberish) and a public address (that looks like "pyrintest:" followed by a bunch of gibberish). For example, the output could look like this:
     ```
     2023-06-25 18:00:58.677+00:00 [INFO ] Connected to RPC
     2023-06-25 18:00:58.677+00:00 [INFO ] Generated private key aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce and address pyrintest:qzlpwt49f0useql6w0tzpnf8k2symdv5tu2x2pe9r9nvngw8mvx57q0tt9lr5. Send some funds to this address and rerun rothschild with `--private-key aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce`
      ```
     Here, the private key is ```aa1c554386218eb28c4bsf6a02e5943799cf951dac7301324d88dec2d0119fce``` and the address is ```pyrintest:qzlpwt49f0useql6w0tzpnf8k2symdv5tu2x2pe9r9nvngw8mvx57q0tt9lr5```
...
Like pyrind, the Rothschild window should remain open and undisturbed.

For mining, grab `pyrin-miner` from within the latest [Release](https://github.com/elichai/pyrin-miner/releases) and run it with the following flags (**this is currently the only miner that supports testnet-11**):
    ```
    pyrin-miner --testnet --mining-address <address> -p 16210 -t 1
    ```
...
Like the Pyrind and Rothschild windows, the miner window should also be left undisturbed, and closing it will stop the mining.

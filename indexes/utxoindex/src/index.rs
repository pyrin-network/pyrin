...
use pyrin_consensus_core::{tx::ScriptPublicKeys, utxo::utxo_diff::UtxoDiff, BlockHashSet};
use pyrin_consensusmanager::{ConsensusManager, ConsensusResetHandler};
use pyrin_core::{info, trace};
use pyrin_database::prelude::{StoreError, StoreResult, DB};
use pyrin_hashes::Hash;
use pyrin_index_core::indexed_utxos::BalanceByScriptPublicKey;
use pyrin_utils::arc::ArcExtensions;
...
const RESYNC_CHUNK_SIZE: usize = 2048; //Increased from 1k (used in go-pyrind), for quicker resets, while still having a low memory footprint.
...
impl UtxoIndexApi for UtxoIndex {
    ...
    fn get_all_outpoints(&self) -> StoreResult<std::collections::HashSet<pyrin_consensus_core::tx::TransactionOutpoint>> {

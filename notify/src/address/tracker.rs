...
use pyrin_addresses::{Address, Prefix};
use pyrin_consensus_core::tx::ScriptPublicKey;
...
/// Tracker of a set of [`Address`](pyrin_addresses::Address), indexing and counting registrations
///
/// #### Implementation design
///
/// Each [`Address`](pyrin_addresses::Address) is stored internally as a [`ScriptPubKey`](pyrin_consensus_core::tx::ScriptPublicKey).
...
mod tests {
    ...
    fn create_addresses(start: usize, count: usize) -> Vec<Address> {
        (start..start + count)
            .map(|i| Address::new(Prefix::Mainnet, pyrin_addresses::Version::PubKey, &Uint256::from_u64(i as u64).to_le_bytes()))
            .collect()
    }

...
macro_rules! full_featured {
    ...
    $($(#[$variant_meta:meta])* $variant_name:ident($field_name:path),)*
        ...
        impl std::convert::From<&$name> for pyrin_notify::events::EventType {
            fn from(value: &$name) -> Self {
                match value {
                    $($name::$variant_name(_) => pyrin_notify::events::EventType::$variant_name),*
                }
            }
        }

        impl std::convert::From<&$name> for pyrin_notify::scope::Scope {
            fn from(value: &$name) -> Self {
                match value {
                    $($name::$variant_name(_) => pyrin_notify::scope::Scope::$variant_name(pyrin_notify::scope::[<$variant_name Scope>]::default())),*
                }
            }
        }

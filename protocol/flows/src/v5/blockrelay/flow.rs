...
use pyrin_consensus_core::{api::BlockValidationFutures, block::Block, blockstatus::BlockStatus, errors::block::RuleError};
use pyrin_consensusmanager::{BlockProcessingBatch, ConsensusProxy};
use pyrin_core::debug;
use pyrin_hashes::Hash;
use pyrin_p2p_lib::{
    common::ProtocolError,
    dequeue, dequeue_with_timeout, make_message, make_request,
    pb::{pyrind_message::Payload, InvRelayBlockMessage, RequestBlockLocatorMessage, RequestRelayBlocksMessage},
    IncomingRoute, Router, SharedIncomingRoute,
};
use pyrin_utils::channel::{JobSender, JobTrySendError as TrySendError};
...
impl HandleRelayInvsFlow {
    ...
    async fn check_orphan_resolution_range(
        ...
        // with current syncer-side implementations (in both go-pyrin and this codebase) we could query only the last one,

...
use pyrin_consensusmanager::ConsensusProxy;
...
use pyrin_p2p_lib::{
    ...
    pb::{pyrind_message::Payload, RequestTransactionsMessage, TransactionNotFoundMessage},
...
impl RelayTransactionsFlow {
    ...
    pub fn invs_channel_size() -> usize {
        ...
        // Note: in go-pyrind we have 10,000 for this channel combined with tx channel.
        4096
    }
            ...
            match res {
                ...
                | Err(MiningManagerError::MempoolError(RuleError::RejectNonStandard(..))) => {
                    ...
                    if self.spam_counter % 100 == 0 {
                        pyrin_core::warn!("Peer {} has shared {} spam/non-standard txs ({:?})", self.router, self.spam_counter, res);
                    }
                }
                Err(_) => {}
            }

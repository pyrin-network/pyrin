use crate::{convert::error::ConversionError, core::peer::PeerKey, PyrindMessagePayloadType};
use pyrin_consensus_core::errors::{block::RuleError, consensus::ConsensusError, pruning::PruningImportError};
use pyrin_mining_errors::manager::MiningManagerError;
...
pub enum ProtocolError {
    ...
    UnexpectedMessage(&'static str, Option<PyrindMessagePayloadType>),
    ...
    IncomingRouteCapacityReached(PyrindMessagePayloadType, String),
    ...
    NoRouteForMessageType(PyrindMessagePayloadType),
...
/// Wraps an inner payload message into a valid `PyrindMessage`.
/// Usage:
/// ```ignore
...
macro_rules! make_message {
    ...
    ($pattern:path, $msg:expr, $response_id:expr, $request_id: expr) => {{
        $crate::pb::PyrindMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: $request_id }
    }};
}
...
macro_rules! make_response {
    ($pattern:path, $msg:expr, $response_id:expr) => {{
        $crate::pb::PyrindMessage { payload: Some($pattern($msg)), response_id: $response_id, request_id: 0 }
    }};
}
...
macro_rules! make_request {
    ($pattern:path, $msg:expr, $request_id:expr) => {{
        $crate::pb::PyrindMessage { payload: Some($pattern($msg)), response_id: 0, request_id: $request_id }
    }};
}

/// Macro to extract a specific payload type from an `Option<pb::PyrindMessage>`.
/// Usage:
/// ```ignore
...
/// Macro to await a channel `Receiver<pb::PyrindMessage>::recv` call with a default/specified timeout and expect a specific payload type.
/// Usage:
/// ```ignore
...
/// Macro to indefinitely await a channel `Receiver<pb::PyrindMessage>::recv` call and expect a specific payload type (without a timeout).
/// Usage:
/// ```ignore

...
use crate::pb::{
    p2p_client::P2pClient as ProtoP2pClient, p2p_server::P2p as ProtoP2p, p2p_server::P2pServer as ProtoP2pServer, PyrindMessage,
};
...
impl ConnectionHandler {
    ...
    fn outgoing_network_channel_size() -> usize {
        // TODO: this number is taken from go-pyrind and should be re-evaluated
        (1 << 17) + 256
    }
...
impl ProtoP2p for ConnectionHandler {
    type MessageStreamStream = Pin<Box<dyn futures::Stream<Item = Result<PyrindMessage, TonicStatus>> + Send + 'static>>;

    /// Handle the new arriving **server** connections
    async fn message_stream(
        &self,
        request: Request<Streaming<PyrindMessage>>,

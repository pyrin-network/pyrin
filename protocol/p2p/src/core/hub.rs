use crate::{common::ProtocolError, pb::PyrindMessage, ConnectionInitializer, Peer, Router};
...
impl Hub {
    ...
    pub async fn send(&self, peer_key: PeerKey, msg: PyrindMessage) -> Result<bool, ProtocolError> {
    ...
    pub async fn broadcast(&self, msg: PyrindMessage) {
    ...
    pub async fn broadcast_to_some_peers(&self, msg: PyrindMessage, num_peers: usize) {
        assert!(num_peers > 0);
    ...
    pub async fn broadcast_many(&self, msgs: Vec<PyrindMessage>) {
        if msgs.is_empty() {
            return;
        }
        let peers = self.peers.read().values().cloned().collect::<Vec<_>>();

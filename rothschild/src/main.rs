...
use pyrin_core::{info, pyrind_env::version, time::unix_now, warn};
use pyrin_grpc_client::{ClientPool, GrpcClient};
use pyrin_notify::subscription::context::SubscriptionContext;
use pyrin_rpc_core::{api::rpc::RpcApi, notify::mode::NotificationMode};
...
#[tokio::main]
async fn main() {
    pyrin_core::log::init_logger(None, "");
    let args = Args::parse();
    ...
    let schnorr_key = if let Some(private_key_hex) = args.private_key {
        ...
        let pyrin_addr = Address::new(ADDRESS_PREFIX, ADDRESS_VERSION, &pk.x_only_public_key().0.serialize());
        info!(
            "Generated private key {} and address {}. Send some funds to this address and rerun rothschild with `--private-key {}`",
            sk.display_secret(),
            String::from(&pyrin_addr),
            sk.display_secret()
        );
        return;
    };

    let pyrin_addr = Address::new(ADDRESS_PREFIX, ADDRESS_VERSION, &schnorr_key.x_only_public_key().0.serialize());

    rayon::ThreadPoolBuilder::new().num_threads(args.threads as usize).build_global().unwrap();

    info!("Using Rothschild with private key {} and address {}", schnorr_key.display_secret(), String::from(&pyrin_addr));
...
\tMedian time: {}, \n\tDAA score: {}, \n\tPruning point: {}, \n\tTips: {}, \n\t{} virtual parents: ...{}, \n\tCoinbase maturity: {}",
    ...
    let mut utxos = refresh_utxos(&rpc_client, pyrin_addr.clone(), &mut pending, coinbase_maturity).await;
    ...
    loop {
        ...
        if !has_funds || now - last_refresh > 60_000 {
            ...
            utxos = refresh_utxos(&rpc_client, pyrin_addr.clone(), &mut pending, coinbase_maturity).await;
...
async fn refresh_utxos(
    ...
    populate_pending_outpoints_from_mempool(rpc_client, pyrin_addr.clone(), pending).await;
    fetch_spendable_utxos(rpc_client, pyrin_addr, coinbase_maturity, pending).await
}
...
async fn populate_pending_outpoints_from_mempool(
    ...
    let entries = rpc_client.get_mempool_entries_by_addresses(vec![pyrin_addr], true, false).await.unwrap();
    let now = unix_now();
...
async fn fetch_spendable_utxos(
    ...
    let resp = rpc_client.get_utxos_by_addresses(vec![pyrin_addr]).await.unwrap();
    ...
    let txs = selected_utxos_groups
        ...
        .map(|utxo_option| {
            if let Some((selected_utxos, selected_amount)) = utxo_option {
                let tx = generate_tx(schnorr_key, &selected_utxos, selected_amount, num_outs, &pyrin_addr);
...
fn generate_tx(
    ...
    let script_public_key = pay_to_address_script(pyrin_addr);
    let inputs = utxos
        .iter()

...
use pyrin_notify::{listener::ListenerId, scope::Scope, subscription::Command};
use std::sync::Arc;
...
pub trait RpcApi: Sync + Send + AnySync {
    ...
    /// Requests the list of known pyrind addresses in the current network (mainnet, testnet, etc.)

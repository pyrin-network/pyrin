...
use pyrin_consensus_core::block::{Block, MutableBlock};
    ...
    fn try_from(item: &RpcBlock) -> RpcResult<Self> {
        Ok(Self {
            ...
            transactions: Arc::new(
                item.transactions
                    .iter()
                    .map(pyrin_consensus_core::tx::Transaction::try_from)
                    .collect::<RpcResult<Vec<pyrin_consensus_core::tx::Transaction>>>()?,
            ),
        })
    }
}

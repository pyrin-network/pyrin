...
use pyrin_consensus_core::tx::{Transaction, TransactionInput, TransactionOutput};
...
impl From<&Transaction> for RpcTransaction {
    fn from(item: &Transaction) -> Self {
        Self {
            ...
            mass: item.mass(),
            // TODO: Implement a populating process inspired from pyrind\app\rpc\rpccontext\verbosedata.go
            verbose_data: None,
        }
    }
}
...
impl TryFrom<&RpcTransaction> for Transaction {
    ...
    fn try_from(item: &RpcTransaction) -> RpcResult<Self> {
        let transaction = Transaction::new(
            ...
            item.inputs
                .iter()
                .map(pyrin_consensus_core::tx::TransactionInput::try_from)
                .collect::<RpcResult<Vec<pyrin_consensus_core::tx::TransactionInput>>>()?,
            item.outputs
                .iter()
                .map(pyrin_consensus_core::tx::TransactionOutput::try_from)
                .collect::<RpcResult<Vec<pyrin_consensus_core::tx::TransactionOutput>>>()?,
            item.lock_time,

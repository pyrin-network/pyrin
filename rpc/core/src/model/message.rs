...
use pyrin_consensus_core::api::stats::BlockCount;
use pyrin_core::debug;
use pyrin_notify::subscription::{context::SubscriptionContext, single::UtxosChangedSubscription, Command};
...
pub struct GetBlockTemplateResponse {
    ...
    /// Whether pyrind thinks that it's synced.
    /// Callers are discouraged (but not forbidden) from solving blocks when pyrind is not synced.
    /// That is because when pyrind isn't in sync with the rest of the network there's a high
...
//
// This call is only available when this pyrind was started with `--utxoindex`
//

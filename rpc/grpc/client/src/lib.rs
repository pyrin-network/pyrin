...
use pyrin_core::{debug, error, trace};
use pyrin_grpc_core::{
    channel::NotificationChannel,
    ops::PyrindPayloadOps,
    protowire::{pyrind_request, rpc_client::RpcClient, GetInfoRequestMessage, PyrindRequest, PyrindResponse},
...
use pyrin_utils::{channel::Channel, triggers::DuplexTrigger};
...
impl RpcApi for GrpcClient {
    ...
    //     self.inner.call(PyrindPayloadOps::SubmitBlock, request).await?.as_ref().try_into()
    // }
...
type PyrindRequestSender = async_channel::Sender<PyrindRequest>;
type PyrindRequestReceiver = async_channel::Receiver<PyrindRequest>;
...
/// Data flow:
/// ```
/// //   PyrindRequest -> request_send -> stream -> PyrindResponse
/// ```
///
/// Execution flow:
/// ```
...
struct Inner {
    ...
    // Sending to server
    request_sender: PyrindRequestSender,
    request_receiver: PyrindRequestReceiver,
...
impl Inner {
    ...
    async fn try_connect(
        ...
        request_timeout: u64,
        counters: Arc<TowerConnectionCounters>,
    ) -> Result<(Streaming<PyrindResponse>, ServerFeatures)> {
        // gRPC endpoint
        ...
        // Actual PyrindRequest to PyrindResponse stream
        let mut stream: Streaming<PyrindResponse> = client.message_stream(request_stream).await?.into_inner();
    ...
    async fn call(&self, op: PyrindPayloadOps, request: impl Into<PyrindRequest>) -> Result<PyrindResponse> {
        // Calls are only allowed if the client is connected to the server
        if self.is_connected() {
            let id = u64::from_le_bytes(rand::random::<[u8; 8]>());
            let mut request: PyrindRequest = request.into();
            request.id = id;
    ...
    fn spawn_response_receiver_task(self: Arc<Self>, mut stream: Streaming<PyrindResponse>) {
        // Note: self is a cloned Arc here so that it can be used in the spawned task.
    ...
    async fn start_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        let request = pyrind_request::Payload::from_notification_type(&scope, Command::Start);
    ...
    async fn stop_notify_to_client(&self, scope: Scope) -> RpcResult<()> {
        if self.handle_stop_notify() {
            let request = pyrind_request::Payload::from_notification_type(&scope, Command::Stop);
            self.call((&request).into(), request).await?;
        }
        Ok(())
    }

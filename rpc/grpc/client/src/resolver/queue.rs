use crate::{
    error::{Error, Result},
    resolver::{matcher::Matcher, PyrindResponseReceiver, PyrindResponseSender, Resolver},
};
...
impl Pending {
    fn new(op: PyrindPayloadOps, request: PyrindRequest, sender: PyrindResponseSender) -> Self {
        Self { timestamp: Instant::now(), op, request, sender }
    }

    fn is_matching(&self, response: &PyrindResponse, response_op: PyrindPayloadOps) -> bool {
...
impl Resolver for QueueResolver {
    ...
    fn handle_response(&self, response: PyrindResponse) {
        let response_op: PyrindPayloadOps = response.payload.as_ref().unwrap().try_into().expect("response is not a notification");

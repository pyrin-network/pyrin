...
use pyrin_rpc_core::{RpcError, RpcHash};
use std::str::FromStr;
...
from!(item: &pyrin_rpc_core::RpcBlock, protowire::RpcBlock, {
    Self {
...
from!(item: &pyrin_rpc_core::RpcBlockVerboseData, protowire::RpcBlockVerboseData, {
    Self {
...
try_from!(item: &protowire::RpcBlock, pyrin_rpc_core::RpcBlock, {
    Self {
        header: item
            .header
            .as_ref()
            ...
            .try_into()?,
        transactions: item.transactions.iter().map(pyrin_rpc_core::RpcTransaction::try_from).collect::<Result<Vec<_>, _>>()?,
        verbose_data: item.verbose_data.as_ref().map(pyrin_rpc_core::RpcBlockVerboseData::try_from).transpose()?,
    }
});

try_from!(item: &protowire::RpcBlockVerboseData, pyrin_rpc_core::RpcBlockVerboseData, {
    Self {
        ...
        transaction_ids: item
            ...
            .collect::<Result<Vec<pyrin_rpc_core::RpcHash>, faster_hex::Error>>()?,
        is_header_only: item.is_header_only,
        blue_score: item.blue_score,

use crate::protowire::{
    pyrind_response::Payload, BlockAddedNotificationMessage, PyrindResponse, NewBlockTemplateNotificationMessage, RpcNotifyCommand,
};
use crate::protowire::{
...
use pyrin_rpc_core::{Notification, RpcError, RpcHash};
use std::str::FromStr;
use std::sync::Arc;
...
from!(item: &pyrin_rpc_core::Notification, PyrindResponse, { Self { id: 0, payload: Some(item.into()) } });

from!(item: &pyrin_rpc_core::Notification, Payload, {
    match item {
...
from!(item: &pyrin_rpc_core::BlockAddedNotification, BlockAddedNotificationMessage, { Self { block: Some((&*item.block).into()) } });

from!(&pyrin_rpc_core::NewBlockTemplateNotification, NewBlockTemplateNotificationMessage);

from!(item: &pyrin_rpc_core::VirtualChainChangedNotification, VirtualChainChangedNotificationMessage, {
    Self {
...
from!(item: &pyrin_rpc_core::FinalityConflictNotification, FinalityConflictNotificationMessage, {
    Self { violating_block_hash: item.violating_block_hash.to_string() }
});

from!(item: &pyrin_rpc_core::FinalityConflictResolvedNotification, FinalityConflictResolvedNotificationMessage, {
    Self { finality_block_hash: item.finality_block_hash.to_string() }
});

from!(item: &pyrin_rpc_core::UtxosChangedNotification, UtxosChangedNotificationMessage, {
    Self {
...
from!(item: &pyrin_rpc_core::SinkBlueScoreChangedNotification, SinkBlueScoreChangedNotificationMessage, {
    Self { sink_blue_score: item.sink_blue_score }
});

from!(item: &pyrin_rpc_core::VirtualDaaScoreChangedNotification, VirtualDaaScoreChangedNotificationMessage, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

from!(&pyrin_rpc_core::PruningPointUtxoSetOverrideNotification, PruningPointUtxoSetOverrideNotificationMessage);
...
try_from!(item: &PyrindResponse, pyrin_rpc_core::Notification, {
    item.payload
        .as_ref()
        .ok_or_else(|| RpcError::MissingRpcFieldError("PyrindResponse".to_string(), "payload".to_string()))?
        .try_into()?
});

try_from!(item: &Payload, pyrin_rpc_core::Notification, {
    match item {
...
try_from!(item: &BlockAddedNotificationMessage, pyrin_rpc_core::BlockAddedNotification, {
    Self {
        block: Arc::new(
            item.block
                .as_ref()
...
try_from!(&NewBlockTemplateNotificationMessage, pyrin_rpc_core::NewBlockTemplateNotification);

try_from!(item: &VirtualChainChangedNotificationMessage, pyrin_rpc_core::VirtualChainChangedNotification, {
    Self {
...
try_from!(item: &FinalityConflictNotificationMessage, pyrin_rpc_core::FinalityConflictNotification, {
    Self { violating_block_hash: RpcHash::from_str(&item.violating_block_hash)? }
});

try_from!(item: &FinalityConflictResolvedNotificationMessage, pyrin_rpc_core::FinalityConflictResolvedNotification, {
    Self { finality_block_hash: RpcHash::from_str(&item.finality_block_hash)? }
});

try_from!(item: &UtxosChangedNotificationMessage, pyrin_rpc_core::UtxosChangedNotification, {
    Self {
...
try_from!(item: &SinkBlueScoreChangedNotificationMessage, pyrin_rpc_core::SinkBlueScoreChangedNotification, {
    Self { sink_blue_score: item.sink_blue_score }
});

try_from!(item: &VirtualDaaScoreChangedNotificationMessage, pyrin_rpc_core::VirtualDaaScoreChangedNotification, {
    Self { virtual_daa_score: item.virtual_daa_score }
});

try_from!(&PruningPointUtxoSetOverrideNotificationMessage, pyrin_rpc_core::PruningPointUtxoSetOverrideNotification);

...
use pyrin_notify::scope::{NewBlockTemplateScope, Scope};
use pyrin_rpc_core::api::rpc::RpcApi;
use pyrin_utils::networking::{ContextualNetAddress, NetAddress};
use std::sync::Arc;

#[tokio::test]
async fn test_client_server_sanity_check() {
    pyrin_core::log::try_init_logger("info, pyrin_grpc_core=trace, pyrin_grpc_server=trace, pyrin_grpc_client=trace");

...
struct RpcTable {
    ...
    pyrind_request_type: Expr,
    pyrind_response_type: Expr,
    payload_ops: Expr,
    handlers: ExprArray,
}
    ...
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        if parsed.len() != 7 {
            return Err(Error::new_spanned(parsed,
                "usage: build_grpc_server_interface!(server_context, ServerContextType, ConnectionType, PyrindRequestType, PyrindResponseType, PyrindPayloadOps, [GetInfo, ..])".to_string()));
        }
        ...
        let pyrind_request_type = iter.next().unwrap().clone();
        let pyrind_response_type = iter.next().unwrap().clone();
...
impl ToTokens for RpcTable {
    fn to_tokens(&self, tokens: &mut TokenStream) {
        ...
        let pyrind_request_type = &self.pyrind_request_type;
        let pyrind_response_type = &self.pyrind_response_type;
        ...
        for handler in self.handlers.elems.iter() {
            ...
            match is_subscription {
                false => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #pyrind_request_type, #pyrind_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, _: #connection_ctx_type, request: #pyrind_request_type| {
                                Box::pin(async move {
                                    let mut response: #pyrind_response_type = match request.payload {
                ...
                true => {
                    targets.push(quote! {
                        #payload_ops::#handler => {
                            let method: Method<#server_ctx_type, #connection_ctx_type, #pyrind_request_type, #pyrind_response_type> =
                            Method::new(|server_ctx: #server_ctx_type, connection: #connection_ctx_type, request: #pyrind_request_type| {
                                Box::pin(async move {
                                    let mut response: #pyrind_response_type = match request.payload {
                                        Some(Payload::#request_type(ref request)) => {
                                            match pyrin_rpc_core::#fallback_request_type::try_from(request) {
                                                Ok(request) => {

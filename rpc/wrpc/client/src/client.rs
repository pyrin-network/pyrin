...
pub use pyrin_rpc_macros::build_wrpc_client_interface;
use std::fmt::Debug;
...
impl Inner {
    pub fn new(encoding: Encoding, url: Option<&str>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Inner> {
        // log_trace!("Pyrin wRPC::{encoding} connecting to: {url}");
        ...
        .for_each(|notification_op| {
            ...
            interface.notification(
                notification_op,
                workflow_rpc::client::Notification::new(move |notification: pyrin_rpc_core::Notification| {
                    ...
                    Box::pin(async move {
                        ...
                        if notification_sender.receiver_count() > 1 {
                            ...
                            log_warn!("WARNING: Pyrin RPC notification is not consumed by user: {:?}", notification);
                        }
                        Ok(())
                    })
                }),
...
/// [`PyrinRpcClient`] allows connection to the Pyrin wRPC Server via
...
///
#[derive(Clone)]
pub struct PyrinRpcClient {
    inner: Arc<Inner>,
}

impl Debug for PyrinRpcClient {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("PyrinRpcClient").field("url", &self.url()).field("connected", &self.is_connected()).finish()
    }
}

impl PyrinRpcClient {
    /// Create a new `PyrinRpcClient` with the given Encoding and URL
    pub fn new(
        encoding: Encoding,
        url: Option<&str>,
    ...
    ) -> Result<PyrinRpcClient> {
        Self::new_with_args(encoding, url, resolver, network_id, subscription_context)
        // FIXME
        // pub fn new(encoding: Encoding, url: &str, ) -> Result<PyrinRpcClient> {
    ...
    pub fn new_with_args(
        ...
        let client = PyrinRpcClient { inner };
        ...
        // let client = PyrinRpcClient { inner, notifier, notification_mode };
            ...
            loop {
                select! {
                    ...
                    msg = notification_relay_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            // inner.rpc_ctl.notify(msg).await.expect("(PyrinRpcClient) rpc_ctl.notify() error");
                    ...
                    msg = wrpc_ctl_channel.receiver.recv().fuse() => {
                        if let Ok(msg) = msg {
                            match msg {
                                WrpcCtl::Connect => {
                                    inner.rpc_ctl.signal_open().await.expect("(PyrinRpcClient) rpc_ctl.signal_open() error");
                                }
                                WrpcCtl::Disconnect => {
                                    inner.rpc_ctl.signal_close().await.expect("(PyrinRpcClient) rpc_ctl.signal_close() error");
                                }
                            }
                        } else {
...
#[async_trait]
impl RpcApi for PyrinRpcClient {
    //

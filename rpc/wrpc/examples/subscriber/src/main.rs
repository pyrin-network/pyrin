// Basic example of a Pyrin wRPC client that connects to a node
...
// Pyrin RPC primitives
use pyrin_wrpc_client::prelude::*;
// reuse wRPC Result type for convenience
use pyrin_wrpc_client::result::Result;
...
impl Listener {
    pub fn try_new(network_id: NetworkId, url: Option<String>) -> Result<Self> {
        ...
        // Create a basic Pyrin RPC client instance using Borsh encoding.
        let client = Arc::new(PyrinRpcClient::new_with_args(WrpcEncoding::Borsh, url.as_deref(), resolver, Some(network_id), None)?);

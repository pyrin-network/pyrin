...
use pyrin_rpc_core::api::ops::RpcApiOps;
...
/// ### PyrinRpcHandler
///
/// [`PyrinRpcHandler`] is a handler struct that implements the [`RpcHandler`] trait
...
///
pub struct PyrinRpcHandler {
    pub server: Server,
    pub options: Arc<Options>,
}

impl PyrinRpcHandler {
    pub fn new(
        tasks: usize,
        ...
        PyrinRpcHandler { server: Server::new(tasks, encoding, core_service, options.clone()), options }
    }
}

#[async_trait]
impl RpcHandler for PyrinRpcHandler {
    ...
    async fn handshake(
        ...
        //     sender,
        //     receiver,
        //     Box::pin(|msg| if msg != "pyrin" { Err(WebSocketError::NegotiationFailure) } else { Ok(()) }),
        // )
        // .await
...
pub struct WrpcService {
    ...
    options: Arc<Options>,
    server: RpcServer,
    rpc_handler: Arc<PyrinRpcHandler>,
...
impl WrpcService {
    ...
    pub fn new(
        ...
        let rpc_handler = Arc::new(PyrinRpcHandler::new(tasks, *encoding, core_service, options.clone()));

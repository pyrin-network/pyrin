...
use crate::imports::*;
use js_sys::Array;
pub use pyrin_rpc_macros::declare_typescript_wasm_interface as declare;
use pyrin_wrpc_client::node::NodeDescriptor;
use pyrin_wrpc_client::Resolver as NativeResolver;
use serde::ser;
...
///
/// Resolver is a client for obtaining public Pyrin wRPC URL.
///
/// Resolver queries a list of public Pyrin Resolver URLs using HTTP to fetch
...
#[wasm_bindgen]
impl Resolver {
    /// List of public Pyrin Resolver URLs.
    #[wasm_bindgen(getter)]
    pub fn urls(&self) -> ResolverArrayT {
        Array::from_iter(self.resolver.urls().iter().map(|v| JsValue::from(v.as_str()))).unchecked_into()
    }

    /// Fetches a public Pyrin wRPC endpoint for the given encoding and network identifier.
    ...
    /// Fetches a public Pyrin wRPC endpoint URL for the given encoding and network identifier.
    ...
    /// Connect to a public Pyrin wRPC endpoint for the given encoding and network identifier

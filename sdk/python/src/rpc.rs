...
use pyrin_notify::scope::{BlockAddedScope, FinalityConflictResolvedScope, FinalityConflictScope, NewBlockTemplateScope, PruningPointUtxoSetOverrideScope, Scope, SinkBlueScoreChangedScope, UtxosChangedScope, VirtualChainChangedScope, VirtualDaaScoreChangedScope};
use pyrin_rpc_core::{Notification, RpcAddress, RpcBlock, RpcContextualPeerAddress, RpcExtraData, RpcHash, RpcIpAddress, RpcSubnetworkId};
use pyrin_rpc_core::api::ctl::RpcState;
use pyrin_rpc_core::notify::connection::{ChannelConnection, ChannelType};
use pyrin_wallet_core::prelude::PyrinRpcClient;
use pyrin_wallet_core::rpc::WrpcEncoding;
...
impl RPC {
    ...
    pub fn connect<'a>(&mut self, py: Python<'a>) -> PyResult<&'a PyAny> {
        ...
        self.client = Some(PyrinRpcClient::new(WrpcEncoding::Borsh, Some(url.as_str().as_ref()), None, Some(NetworkId::new(NetworkType::Mainnet)), None).unwrap());
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            // let client = Arc::new(
            //     PyrinRpcClient::new(, url.as_deref(), resolver.clone().map(Into::into), network_id, None)
    ...
    pub fn submit_block<'a>(&mut self, py: Python<'a>, block_dict: &PyDict, allow_non_daa_blocks: bool) -> PyResult<&'a PyAny> {
        ...
        pyo3_asyncio::tokio::future_into_py(py, async move {
            ...
            let result = match result {
                pyrin_rpc_core::SubmitBlockReport::Success => 0,
                pyrin_rpc_core::SubmitBlockReport::Reject(pyrin_rpc_core::SubmitBlockRejectReason::BlockInvalid) => 1,
                pyrin_rpc_core::SubmitBlockReport::Reject(pyrin_rpc_core::SubmitBlockRejectReason::IsInIBD) => 2,
                pyrin_rpc_core::SubmitBlockReport::Reject(pyrin_rpc_core::SubmitBlockRejectReason::RouteIsFull) => 0,
            };

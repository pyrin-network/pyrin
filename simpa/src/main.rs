...
use pyrin_core::{info, task::service::AsyncService, task::tick::TickService, time::unix_now, trace, warn};
use pyrin_database::prelude::ConnBuilder;
use pyrin_database::{create_temp_db, load_existing_db};
use pyrin_hashes::Hash;
use pyrin_perf_monitor::{builder::Builder, counters::CountersSnapshot};
use pyrin_utils::fd_budget;
use simulator::network::PyrinNetworkSimulator;
use std::{collections::VecDeque, sync::Arc, time::Duration};

pub mod simulator;

/// Pyrin Network Simulator
#[derive(Parser, Debug)]
...
fn main() {
    ...
    pyrin_core::log::init_logger(None, &args.log_level);
...
fn main_impl(mut args: Args) {
    ...
    let (consensus, _lifetime) = if let Some(input_dir) = args.input_dir {
        ...
        let mut sim = PyrinNetworkSimulator::new(args.delay, args.bps, args.target_blocks, config.clone(), args.output_dir);
...
fn apply_args_to_consensus_params(args: &Args, params: &mut Params) {
    ...
    if args.testnet11 {
        info!(
            "Using pyrin-testnet-11 configuration (GHOSTDAG K={}, DAA window size={}, Median time window size={})",
            params.ghostdag_k,
...
mod tests {
    ...
    fn test_pruning_via_simpa() {
        ...
        pyrin_core::log::try_init_logger(&args.log_level);
        // As we log the panic, we want to set it up after the logger
        pyrin_core::panic::configure_panic();
        main_impl(args);
    }

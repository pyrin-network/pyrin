...
use pyrin_consensus_core::{api::ConsensusApi, blockhash};
...
async fn test_concurrent_pipeline() {
    ...
    for (hash, parents) in blocks {
        ...
        let b: pyrin_consensus_core::block::Block = consensus.build_block_with_parents(hash, parents).to_immutable();
        let results = join!(

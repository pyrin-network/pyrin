...
use pyrin_consensus_core::{constants::LEOR_PER_PYRIN, network::NetworkType, tx::Transaction};
use pyrin_core::{debug, info};
use pyrin_notify::{
    listener::ListenerId,
    scope::{NewBlockTemplateScope, Scope},
};
use pyrin_rpc_core::{api::rpc::RpcApi, Notification, RpcError};
...
/// `cargo test --release --package pyrin-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn bench_bbt_latency() {
    pyrin_core::log::try_init_logger("info,pyrin_core::time=debug,pyrin_mining::monitor=debug");
    ...
    */
    ...
    let pay_address =
        Address::new(network.network_type().into(), pyrin_addresses::Version::PubKey, &pk.x_only_public_key().0.serialize());
    ...
    let submit_block_pool_tasks = submit_block_pool.start(|c, block| async move {
        let _sw = pyrin_core::time::Stopwatch::<500>::with_threshold("sb");
        let response = c.submit_block(block, false).await.unwrap();
        assert_eq!(response.report, pyrin_rpc_core::SubmitBlockReport::Success);
        false
    });
    ...
    let submit_tx_pool_tasks = submit_tx_pool.start(|c, (i, tx)| async move {
        match c.submit_transaction(tx.as_ref().into(), false).await {
            ...
            Err(RpcError::General(msg)) if msg.contains("orphan") => {
                pyrin_core::warn!("\n\n\n{msg}\n\n");
                pyrin_core::warn!("Submitted {} transactions, exiting tx submit loop", i);
                return true;
            }
            Err(e) => panic!("{e}"),
        }
        ...
        while let Ok(notification) = notification_rx.recv().await {
            match notification {
                Notification::NewBlockTemplate(_) => {
                    ...
                    // let _sw = pyrin_core::time::Stopwatch::<500>::with_threshold("bbt");
                    *current_template.lock() = cc.get_block_template(pac.clone(), vec![]).await.unwrap();
                }
                _ => panic!(),
            }
            if !exec.load(Ordering::Relaxed) {
                pyrin_core::warn!("Test is over, stopping miner receiver loop");
                break;
            }
        }
        pyrin_core::warn!("Miner receiver loop task exited");
    });
    ...
    let miner_loop_task = tokio::spawn(async move {
        for i in 0..BLOCK_COUNT {
            ...
            if !exec.load(Ordering::Relaxed) {
                pyrin_core::warn!("Test is over, stopping miner loop");
                break;
            }
        }
        ...
        pyrin_core::warn!("Miner loop task exited");
    });
    ...
    let tx_sender_task = tokio::spawn(async move {
        ...
        pyrin_core::warn!("Tx sender task, waiting for mempool to drain..");
        loop {
            if !exec.load(Ordering::Relaxed) {
                break;
            }
        ...
        exec.store(false, Ordering::Relaxed);
        pyrin_core::warn!("Tx sender task exited");
    });
...
/// Run this benchmark with the following command line:
/// `cargo test --release --package pyrin-testing-integration --lib --features devnet-prealloc -- mempool_benchmarks::bench_bbt_latency_2 --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]

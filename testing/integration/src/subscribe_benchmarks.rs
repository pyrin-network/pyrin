...
use pyrin_consensus::params::Params;
use pyrin_consensus_core::network::{NetworkId, NetworkType};
use pyrin_core::{info, task::tick::TickService, trace};
use pyrin_math::Uint256;
use pyrin_notify::scope::VirtualDaaScoreChangedScope;
use pyrin_rpc_core::api::rpc::RpcApi;
...
fn create_client_addresses(index: usize, network_id: &NetworkId) -> Vec<Address> {
    ...
    (min_address..max_address)
        .map(|x| Address::new((*network_id).into(), pyrin_addresses::Version::PubKey, &Uint256::from_u64(x as u64).to_le_bytes()))
        .collect_vec()
}

/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::utxos_changed_subscriptions_sanity_check --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
async fn utxos_changed_subscriptions_sanity_check() {
    init_allocator_with_default_settings();
    pyrin_core::log::try_init_logger(
        "INFO, pyrin_core::time=debug, pyrin_rpc_core=debug, pyrin_grpc_client=debug, pyrin_notify=info, pyrin_notify::address::tracker=debug, pyrin_notify::listener=debug, pyrin_notify::subscription::single=debug, pyrin_mining::monitor=debug, pyrin_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    pyrin_core::panic::configure_panic();
    ...
    // Fold-up
    //
    pyrin_core::info!("Signal the daemon to shutdown");
    client.shutdown().await.unwrap();
    pyrin_core::warn!("Disconnect the main client");
    client.disconnect().await.unwrap();
    drop(client);

    pyrin_core::warn!("Waiting for the daemon to exit...");
    daemon_process.wait().await.expect("failed to wait for the daemon process");
}

/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_daemon --exact --nocapture --ignored -- --rpc=16610 --p2p=16611 --private-key=a2760251adb5b6e8d4514d23397f1631893e168c33f92ff8a7a24f397d355d62 --max-tracked-addresses=1000000 --utxoindex`
///
...
async fn bench_utxos_changed_subscriptions_daemon() {
    init_allocator_with_default_settings();
    pyrin_core::log::try_init_logger(
        "INFO, pyrin_core::core=trace, pyrin_core::time=debug, pyrin_rpc_core=debug, pyrin_grpc_client=debug, pyrin_notify=info, pyrin_notify::address::tracker=debug, pyrin_notify::listener=debug, pyrin_notify::subscription::single=debug, pyrin_mining::monitor=debug, pyrin_testing_integration::subscribe_benchmarks=trace",
    );
    // As we log the panic, we want to set it up after the logger
    pyrin_core::panic::configure_panic();
...
async fn utxos_changed_subscriptions_client(address_cycle_seconds: u64, address_max_cycles: usize) {
    ...
    let prealloc_address =
        Address::new(NetworkType::Simnet.into(), pyrin_addresses::Version::PubKey, &prealloc_pk.x_only_public_key().0.serialize());
...
/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_a --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_b --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_c --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_d --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]
...
/// `cargo test --package pyrin-testing-integration --lib --features devnet-prealloc -- subscribe_benchmarks::bench_utxos_changed_subscriptions_footprint_e --exact --nocapture --ignored`
#[tokio::test]
#[ignore = "bmk"]

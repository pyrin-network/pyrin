...
impl Task for StatRecorderTask {
    fn start(&self, stop_signal: SingleTrigger) -> Vec<JoinHandle<()>> {
        ...
        let task = tokio::spawn(async move {
            pyrin_core::warn!("Stat recorder task starting...");
            std::fs::create_dir_all(PathBuf::from(&folder)).unwrap();
            {
                let file_path = PathBuf::from(&folder).join(file_name).into_os_string();
                pyrin_core::warn!("Recording memory metrics into file {}", file_path.to_str().unwrap());
                ...
                loop {
                    tokio::select! {
                        ...
                        _ = stop_signal.listener.clone() => {
                            pyrin_core::trace!("Leaving stat recorder loop");
                            break;
                        }
            ...
            pyrin_core::warn!("Stat recorder task exited");
        });
        vec![task]
    }
}

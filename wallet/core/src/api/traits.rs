//!
//! API trait for interfacing with the Pyrin wallet subsystem.
//!
...
pub trait WalletApi: Send + Sync + AnySync {
    ...
    /// If the `filename` is `None`, the wallet opens the default wallet named `pyrin`.
    ///
    ...
    /// applications such as KDX and pyrinnet.io web wallet using a deprecated derivation path

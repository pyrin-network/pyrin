...
mod test {
    ...
    async fn import_golang_single_wallet_test() {
        ...
        let acc = wallet.import_pyrinwallet_golang_single_v1(&import_secret, &wallet_secret, file).await.unwrap();
        assert_eq!(
            acc.receive_address().unwrap(),
            Address::try_from("pyrin:qpuvlauc6a5syze9g70dnxzzvykhkuatsjrx87mxqccqh7kf9kcssdkp9ec7w").unwrap(), // taken from golang impl
        );
    }
    ...
    async fn import_golang_multisig_v1_wallet_test() {
        ...
        let acc = wallet.import_pyrinwallet_golang_multisig_v1(&import_secret, &wallet_secret, file).await.unwrap();
        assert_eq!(
            acc.receive_address().unwrap(),
            Address::try_from("pyrin:pqvgkyjeuxmd8k70egrrzpdz5rqj0acmr6y94mwsltxfp6nc50742295c3998").unwrap(), // taken from golang impl
        );
    }
    ...
    fn deser_golang_wallet_test() {
        ...
        struct EncryptedMnemonicIntermediate {
            #[serde(with = "pyrin_utils::serde_bytes")]
            cipher: Vec<u8>,
            #[serde(with = "pyrin_utils::serde_bytes")]
            salt: Vec<u8>,
        }

    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        if let Some(final_transaction_amount) = self.final_transaction_amount {
            ...
            write!(
                ...
                sompi_to_pyrin_string_with_suffix(final_transaction_amount, &self.network_id),
                sompi_to_pyrin_string_with_suffix(self.aggregated_fees, &self.network_id),
                sompi_to_pyrin_string_with_suffix(total, &self.network_id),
                self.aggregated_utxos,
                transactions
            )?;
        } else {

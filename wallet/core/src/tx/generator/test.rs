...
use crate::{tx::PaymentOutputs, utils::pyrin_to_sompi};
...
impl From<Pyrin> for Sompi {
    fn from(pyrin: Pyrin) -> Self {
        Sompi(pyrin_to_sompi(pyrin.0))
    }
}
    ...
    F: FnOnce(NetworkType) -> Address,
{
    ...
    let utxo_entries: Vec<UtxoEntryReference> = values.into_iter().map(pyrin_to_sompi).map(UtxoEntryReference::simulated).collect();
...
pub(crate) fn change_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("pyrin:qpauqsvk7yf9unexwmxsnmg547mhyga37csh0kj53q6xxgl24ydxjsgzthw5j").unwrap(),
        NetworkType::Testnet => Address::try_from("pyrintest:qqz22l98sf8jun72rwh5rqe2tm8lhwtdxdmynrz4ypwak427qed5juktjt7ju").unwrap(),
...
pub(crate) fn output_address(network_type: NetworkType) -> Address {
    match network_type {
        NetworkType::Mainnet => Address::try_from("pyrin:qrd9efkvg3pg34sgp6ztwyv3r569qlc43wa5w8nfs302532dzj47knu04aftm").unwrap(),
        NetworkType::Testnet => Address::try_from("pyrintest:qqrewmx4gpuekvk8grenkvj2hp7xt0c35rxgq383f6gy223c4ud5s58ptm6er").unwrap(),
        _ => unreachable!("network type not supported"),
    }
}
...
fn test_generator_sweep_two_utxos() -> Result<()> {
    make_generator(test_network_id(), &[10.0, 10.0], &[], Fees::None, change_address, PaymentDestination::Change)
        ...
        .fetch(&Expected {
            is_final: true,
            input_count: 2,
            aggregate_input_value: Pyrin(20.0),
            output_count: 1,
...
fn test_generator_sweep_two_utxos_with_priority_fees_rejection() -> Result<()> {
    let generator =
        make_generator(test_network_id(), &[10.0, 10.0], &[], Fees::sender(Pyrin(5.0)), change_address, PaymentDestination::Change);
    match generator {
...
fn test_generator_compound_200k_10kas_transactions() -> Result<()> {
    generator(test_network_id(), &[10.0; 200_000], &[], Fees::sender(Pyrin(5.0)), [(output_address, Pyrin(190_000.0))].as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_compound_100k_random_transactions() -> Result<()> {
    ...
    let outputs = [(output_address, Pyrin(total - 10.0))];
    generator(test_network_id(), &inputs, &[], Fees::sender(Pyrin(5.0)), outputs.as_slice()).unwrap().harness().validate().finalize();
...
fn test_generator_random_outputs() -> Result<()> {
    ...
    let outputs: Vec<_> = outputs.into_iter().map(|v| (output_address, Pyrin(v))).collect();

    generator(test_network_id(), &[total + 100.0], &[], Fees::sender(Pyrin(5.0)), outputs.as_slice())
        .unwrap()
        .harness()
        .validate()
        .finalize();
...
fn test_generator_dust_1_1() -> Result<()> {
    generator(
        ...
        &[10.0; 20],
        &[],
        Fees::sender(Pyrin(5.0)),
        [(output_address, Pyrin(1.0)), (output_address, Pyrin(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: true,
        input_count: 4,
        aggregate_input_value: Pyrin(40.0),
        output_count: 3,
        priority_fees: FeesExpected::sender(Pyrin(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_2_outputs_2_fees_exclude() -> Result<()> {
    generator(
        ...
        &[],
        Fees::sender(Pyrin(5.0)),
        [(output_address, Pyrin(10.0)), (output_address, Pyrin(1.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
...
fn test_generator_inputs_100_outputs_1_fees_exclude_success() -> Result<()> {
    // generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Pyrin(5.0)), [(output_address, Pyrin(990.0))].as_slice())
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Pyrin(0.0)), [(output_address, Pyrin(990.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
            input_count: 88,
            aggregate_input_value: Pyrin(880.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            is_final: false,
            input_count: 12,
            aggregate_input_value: Pyrin(120.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {
            ...
            output_count: 2,
            // priority_fees: FeesExpected::sender(Pyrin(5.0)),
            priority_fees: FeesExpected::sender(Pyrin(0.0)),
        })
        .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_include_success() -> Result<()> {
    generator(
        ...
        &[1.0; 100],
        &[],
        Fees::receiver(Pyrin(5.0)),
        // [(output_address, Pyrin(100.0))].as_slice(),
        [(output_address, Pyrin(100.0))].as_slice(),
    )
    .unwrap()
    .harness()
    .fetch(&Expected {
        is_final: false,
        input_count: 88,
        aggregate_input_value: Pyrin(88.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        is_final: false,
        input_count: 12,
        aggregate_input_value: Pyrin(12.0),
        output_count: 1,
        priority_fees: FeesExpected::None,
    })
    .fetch(&Expected {
        ...
        output_count: 1,
        priority_fees: FeesExpected::receiver(Pyrin(5.0)),
    })
    .finalize();
...
fn test_generator_inputs_100_outputs_1_fees_exclude_insufficient_funds() -> Result<()> {
    generator(test_network_id(), &[10.0; 100], &[], Fees::sender(Pyrin(5.0)), [(output_address, Pyrin(1000.0))].as_slice())
        .unwrap()
        .harness()
        .fetch(&Expected {
            is_final: false,
...
fn test_generator_inputs_903_outputs_2_fees_exclude() -> Result<()> {
    generator(test_network_id(), &[10.0; 1_000], &[], Fees::sender(Pyrin(5.0)), [(output_address, Pyrin(9_000.0))].as_slice())
        .unwrap()
        .harness()
        .drain(
            10,
        ...
        .fetch(&Expected {
            is_final: false,
            input_count: 21,
            aggregate_input_value: Pyrin(210.0),
            output_count: 1,
            priority_fees: FeesExpected::None,
        })
        .fetch(&Expected {

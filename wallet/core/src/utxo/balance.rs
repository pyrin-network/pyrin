...
export interface IBalance {
    /**
     * Total amount of Pyrin (in SOMPI) available for 
     * spending.
     */
    mature: bigint;
    /**
     * Total amount of Pyrin (in SOMPI) that has been 
     * received and is pending confirmation.
     */
    pending: bigint;
    /**
     * Total amount of Pyrin (in SOMPI) currently 
...
impl From<(Option<&Balance>, &NetworkType, Option<usize>)> for BalanceStrings {
    fn from((balance, network_type, padding): (Option<&Balance>, &NetworkType, Option<usize>)) -> Self {
        let suffix = utils::pyrin_suffix(network_type);
        if let Some(balance) = balance {
            let mut mature = utils::sompi_to_pyrin_string(balance.mature);
            let mut pending = if balance.pending > 0 { Some(utils::sompi_to_pyrin_string(balance.pending)) } else { None };

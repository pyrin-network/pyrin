...
// use pyrin_metrics_core::{Metrics,Metric};
...
impl UtxoProcessor {
    ...
    pub fn rpc_client(&self) -> Option<Arc<PyrinRpcClient>> {
        self.rpc_api().clone().downcast_arc::<PyrinRpcClient>().ok()
    }
    ...
    pub async fn init_state_from_server(&self) -> Result<bool> {
        ...
        // log_trace!("Connected to pyrind: '{server_version}' on '{server_network_id}';  SYNC: {is_synced}  DAA: {virtual_daa_score}");

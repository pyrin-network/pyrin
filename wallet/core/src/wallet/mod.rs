//!
//! Pyrin wallet runtime implementation.
//!
pub mod api;
pub mod args;
pub mod maps;
...
use pyrin_bip32::{ExtendedKey, Language, Mnemonic, Prefix as KeyPrefix, WordCount};
use pyrin_notify::{
    listener::ListenerId,
    scope::{Scope, VirtualDaaScoreChangedScope},
};
use pyrin_wrpc_client::{PyrinRpcClient, Resolver, WrpcEncoding};
...
impl Wallet {
    ...
    pub fn try_with_wrpc(store: Arc<dyn Interface>, resolver: Option<Resolver>, network_id: Option<NetworkId>) -> Result<Wallet> {
        let rpc_client =
            Arc::new(PyrinRpcClient::new_with_args(WrpcEncoding::Borsh, Some("wrpc://127.0.0.1:17110"), resolver, network_id, None)?);

        // pub fn try_with_wrpc(store: Arc<dyn Interface>, network_id: Option<NetworkId>) -> Result<Wallet> {
        //     let rpc_client = Arc::new(PyrinRpcClient::new_with_args(
        //         WrpcEncoding::Borsh,
    ...
    pub fn try_wrpc_client(&self) -> Option<Arc<PyrinRpcClient>> {
        self.try_rpc_api().and_then(|api| api.clone().downcast_arc::<PyrinRpcClient>().ok())
    }
    ...
    pub fn address_prefix(&self) -> Result<pyrin_addresses::Prefix> {
    ...
    pub async fn import_pyrinwallet_golang_single_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        ...
        let prefix = file.xpublic_key.split_at(pyrin_bip32::Prefix::LENGTH).0;
        let prefix = pyrin_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_pyrinwallet_golang_single_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
    ...
    pub async fn import_pyrinwallet_golang_multisig_v0<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        let prefix = first_pub_key.split_at(pyrin_bip32::Prefix::LENGTH).0;
        let prefix = pyrin_bip32::Prefix::try_from(prefix)?;
    ...
    pub async fn import_pyrinwallet_golang_multisig_v1<T: AsRef<[u8]>>(
        self: &Arc<Wallet>,
        import_secret: &Secret,
        wallet_secret: &Secret,
        ...
        all_pub_keys.sort_unstable_by(|left, right| {
            left.split_at(pyrin_bip32::Prefix::LENGTH).1.cmp(right.split_at(pyrin_bip32::Prefix::LENGTH).1)
        });
    ...
    pub async fn import_multisig_with_mnemonic(
        ...
        let mut additional_xpub_keys = additional_xpub_keys
            ...
            .collect::<Result<Vec<_>, pyrin_bip32::Error>>()?;
...
mod test {
    ...
    use pyrin_addresses::{Prefix, Version};
    use pyrin_bip32::{ChildNumber, ExtendedPrivateKey, SecretKey};
    use pyrin_consensus_core::subnets::SUBNETWORK_ID_NATIVE;
    use pyrin_consensus_wasm::{sign_transaction, SignableTransaction, Transaction, TransactionInput, TransactionOutput};
    ...
    async fn wallet_test() -> Result<()> {
        ...
        let address = Address::try_from("pyrintest:qz7ulu4c25dh7fzec9zjyrmlhnkzrg4wmf89q7gzr3gfrsj3uz6xjceef60sd")?;
        ...
        let to_address = Address::try_from("pyrintest:qpakxqlesqywgkq7rg4wyhjd93kmw7trkl3gpa3vd5flyt59a43yyn8vu0w8c")?;
        ...
        let derivation_path =
            gen1::WalletDerivationManager::build_derivate_path(false, 0, None, Some(pyrin_bip32::AddressType::Receive))?;

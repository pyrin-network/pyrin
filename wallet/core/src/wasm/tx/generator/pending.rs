...
use pyrin_consensus_client::{numeric, string};
use pyrin_consensus_client::{ITransaction, Transaction};
use pyrin_wallet_keys::privatekey::PrivateKey;
use pyrin_wrpc_wasm::RpcClient;
    ...
    pub fn sign(&self, js_value: PrivateKeyArrayT) -> Result<()> {
        if let Ok(keys) = js_value.dyn_into::<Array>() {
            let keys = keys
                ...
                .collect::<std::result::Result<Vec<_>, pyrin_wallet_keys::error::Error>>()?;

...
use crate::tx::mass;
use crate::wasm::tx::*;
use pyrin_consensus_client::*;
use pyrin_consensus_core::config::params::Params;
use pyrin_consensus_core::tx as cctx;
use std::sync::Arc;
...
impl MassCalculator {
    ...
    pub fn calc_mass_for_outputs(&self, outputs: JsValue) -> Result<u32> {
        let outputs = outputs
            ...
            .collect::<std::result::Result<Vec<_>, pyrin_consensus_client::error::Error>>()?;

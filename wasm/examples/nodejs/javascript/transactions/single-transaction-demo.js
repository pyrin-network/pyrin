...
(async () => {
    ...
    // An address such as pyrin:qr0lr4ml9fn3chekrqmjdkergxl93l4wrk3dankcgvjq776s9wn9jkdskewva
    const address = keypair.toAddress(networkId);
    console.info(`Full pyrin address: ${address}`);
    ...
    try {
        ...
        if (utxos.length === 0) {
            console.info('Send some pyrin to', address, 'before proceeding with the demo');
            return;
        }
